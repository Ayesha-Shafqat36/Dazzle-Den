...
import { getRelatedProducts } from "../features/recommendation/recommendationSlice";
...
const SingleProduct = () => {
  ...
  useEffect(() => {
    if (productState?.category && productState?.brand && getProductId) {
      dispatch(
        getRelatedProducts({
  ...
  }, [productState, getProductId, dispatch]); 
  const uploadCart = () => {
    if (color === null) {
      ...
      dispatch(
        ...
        if (!result.error) {
          ...
          // Error message will be shown by toast from the reducer
